...
exports.startScraperLifecycle = startScraperLifecycle;
exports.renderGatewayContainer = renderGatewayContainer;
exports.renderScraperConfigurationForm = renderScraperConfigurationForm;
// ============================================================================
// 2. LIFECYCLE POLLING ENGINE
// ============================================================================
function startScraperLifecycle(jobId, onStateChange) {
    if (!jobId)
        return;
...
// ============================================================================
function renderGatewayContainer(targetId, state) {
    ...
    if (state.Error) {
        ...
        currentUiVariant = state.JobData.Status;
    }
...
// ============================================================================
function renderScraperConfigurationForm(containerId, onJobStarted) {
    ...
    function updateFormUI(errorMessage = null, isProcessing = false) {
        container.innerHTML = `
    ...
    `;
        const urlInput = container.querySelector('#target-url');
        if (urlInput) {
        ...
        state.Selectors.forEach((_, index) => {
            const selectorInput = container.querySelector(`#selector-input-${index}`);
            if (selectorInput) {
                selectorInput.addEventListener('input', (e) => {
                    state.Selectors[index].Query = e.target.value;
                });
            }
        });
        // Form submission processing handler
        const form = container.querySelector('#scraper-config-form');
